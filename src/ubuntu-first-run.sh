...
echo "🔒 Rollback log: $ROLLBACK_FILE"
echo "📦 Rollback archive: $ROLLBACK_TAR"
...
update_shell_prompt() {
  ...
  echo "🔁 Backing up $bashrc into rollback tarball..."
  ...
  echo "echo '✅ Restored .bashrc from rollback archive'" >> "$ROLLBACK_FILE"
...
#---------------------------------------------
check_for_rollbacks() {
  echo "🔍 Checking for rollback points in $ROLLBACK_DIR..."
  ...
  if [[ "$restore" =~ ^[Yy]$ ]]; then
    ...
    if [[ "$choice" =~ ^[0-9]+$ && "$choice" -ge 1 && "$choice" -le ${#files[@]} ]]; then
      echo "🚨 Restoring to ${files[$((choice-1))]}..."
      for ((j=${#files[@]}-1; j>=$((choice-1)); j--)); do
        echo "↩️ Running rollback: ${files[$j]}"
        bash "${files[$j]}"
      done
...
run_selected_actions() {
  ...
  if [[ "$update_prompt" =~ ^[Yy]$ ]]; then
    echo "✔ Update shell prompt"
  else
    echo "✘ Skip shell prompt update"
  fi
